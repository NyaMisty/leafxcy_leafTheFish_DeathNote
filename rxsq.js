/*
热项社圈
扫码注册：https://raw.githubusercontent.com/leafTheFish/DeathNote/main/rxsq.png
...
let userCookie = ($.isNode() ? process.env.rxsqCookie : $.getdata('rxsqCookie')) || '';
let userList = []
...
class UserInfo {
    ...
    async getUserName() {
        try {
            let url = `https://s78.yyyyy.run/api/user/index`
            let body = ``
            ...
            if(result.code==1) {
                ...
                console.log(`账号[${this.name}]等级${result.data.level}，${result.data.levelName}，余额${result.data.money}`)
            } else {
    ...
    async userSignData() {
        try {
            let url = `https://s78.yyyyy.run/api/sign/userSignData`
            let body = ``
            let urlObject = populateUrlObject(url,this.token,body)
            await httpRequest('post',urlObject)
            let result = httpResult;
            ...
            //console.log(result)
            if(result.code==1) {
                this.count = result.data.today_count
                console.log(`账号[${this.name}]今天已签到${this.count}次，今天签到积分${result.data.today_coin}，总积分${result.data.coin}`)
            } else {
                console.log(`账号[${this.name}]查询签到次数失败: ${result.msg}`)
            }
        } catch(e) {
            console.log(e)
        } finally {}
    ...
    async get_user_signdata() {
        try {
            let url = `https://s78.yyyyy.run/api/sign/get_user_signdata`
            let body = ``
            let urlObject = populateUrlObject(url,this.token,body)
            await httpRequest('post',urlObject)
        } catch(e) {
    ...
    async userSignIn() {
        try {
            let url = `https://s78.yyyyy.run/api/sign/userSignIn`
            let body = ``
            let urlObject = populateUrlObject(url,this.token,body)
            await httpRequest('post',urlObject)
            let result = httpResult;
            ...
            if(result.code==1) {
                this.count++
                console.log(`账号[${this.name}]签到成功，已签到${this.count}次`)
            } else {
                console.log(`账号[${this.name}]签到失败: ${result.msg}`)
            }
        } catch(e) {
            console.log(e)
        } finally {}
    ...
    async lingqu() {
        try {
            let url = `https://s78.yyyyy.run/api/user/lingqu`
            let body = ``
            let urlObject = populateUrlObject(url,this.token,body)
            await httpRequest('post',urlObject)
            let result = httpResult;
            ...
            if(result.code==1) {
                ...
                console.log(`账号[${this.name}]领取收益失败: ${result.msg}`)
            }
        } catch(e) {
            console.log(e)
        } finally {}
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        let validList = userList.filter(x => x.valid);
        if(validList.length == 0) return;
        
        console.log('\n============ 开始签到 ============')
        for(let i=0; i<SIGN_TIME; i++) {
            let runList = validList.filter(x => x.count < SIGN_TIME)
            if(runList.length == 0) break;
            if(i > 0) {
                console.log('等待一分钟后进行下一次签到...')
                await $.wait(61*1000);
            }
            for(let user of runList) {
                await user.get_user_signdata();
                await user.userSignIn();
            }
        }
        
        if(nowHour >= LINGQU_TIME) {
            console.log('\n============ 领取收益 ============')
...
function populateUrlObject(url,token,body=''){
    ...
    let urlObject = {
        ...
        headers: {
            ...
            'token': token,
            'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 15_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.20(0x18001431) NetType/WIFI Language/zh_CN'
        },
        timeout: 5000,
    }
    if(body) {
